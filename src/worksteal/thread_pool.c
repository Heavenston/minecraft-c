...
struct wt_priv_data {
    struct mcc_worksteal_thread_pool *pool        ;
    size_t                            thread_index;
};

struct mcc_worksteal_thread_pool {
    /**
...
_Atomic(struct mcc_worksteal_thread_pool*) o_global_thread_pool = NULL;

/**
 * Randomly tries all threads on the thread pool to find one with a task to steal
 */
static enum mcc_worksteal_take_result random_steal(struct mcc_worksteal_thread_pool *pool, mcc_worksteal_data_type_t *out) {
...
static struct mcc_worksteal_thread_pool *create_thread_pool() {
    struct mcc_worksteal_thread_pool *new_thread_pool = malloc(sizeof(*new_thread_pool));
 ...
 */
static struct mcc_worksteal_thread_pool *init_global_thread_pool() {
    struct mcc_worksteal_thread_pool *new_thread_pool = create_thread_pool();

    struct mcc_worksteal_thread_pool *expected = NULL;
    if (atomic_compare_exchange_strong(&o_global_thread_pool, &expected, new_thread_pool))
        return new_thread_pool;
    struct mcc_worksteal_thread_pool *pool = atomic_load_explicit(&o_global_thread_pool, memory_order_relaxed);
    assert(pool != NULL);
    return pool;
}

struct mcc_worksteal_thread_pool *mcc_worksteal_thread_pool_global() {
    struct mcc_worksteal_thread_pool *pool = atomic_load_explicit(&o_global_thread_pool, memory_order_relaxed);
    if (!pool)
...
ssize_t get_self_thread_idx(struct mcc_worksteal_thread_pool *pool) {
...
bool mcc_worksteal_thread_pool_is_worker_thread(struct mcc_worksteal_thread_pool *pool) {
    return get_self_thread_idx(pool) != -1;
}

void mcc_worksteal_thread_pool_lock(struct mcc_worksteal_thread_pool *pool) {
    if (mcc_worksteal_thread_pool_is_worker_thread(pool))
        return;
...
void mcc_worksteal_thread_pool_push_task(struct mcc_worksteal_thread_pool *pool, struct mcc_worksteal_thread_pool_task task) {
...
void mcc_worksteal_thread_pool_unlock(struct mcc_worksteal_thread_pool *pool) {
    if (mcc_worksteal_thread_pool_is_worker_thread(pool))
        return;
    #ifndef NDEBUG
    assert(pool->locking_seed_queue_thread_id == gettid());
