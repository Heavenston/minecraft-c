...
static bool polygon_filter_wireframe(const struct mcc_barycentric_coords *coords) {
    // Only pixels near edges are rendered
    const float edge_threshold = 0.02f;
    return coords->u <= edge_threshold || coords->v <= edge_threshold || coords->w <= edge_threshold;
}
...
// Common rasterization function for all polygon modes
static void rasterize_triangle(const struct mcc_rasterization_context *r_context) {
    ...
    for (uint32_t y = 0; y < r_context->r_attachment->height; y++) {
        for (uint32_t x = 0; x < r_context->r_attachment->width; x++) {
            ...
            // Apply polygon mode filter (fill, wireframe, point)
